...
- Compose compiler stability/skippability reports and Compose-aware Perfetto
  traces can now be generated with the evidence collector below.
- Macrobenchmark results remain local/manual and are not trended in default PR
  CI.
- Performance numbers must only be claimed from a completed evidence run on a
  named device or emulator.
- Lower-end-device confidence is not established by compile-only validation.

### Recomposition and jank evidence

The benchmark build includes Compose Runtime Tracing. The Macrobenchmark module
enables full tracing and packages the Perfetto tracing dependencies only in the
test APK, so benchmark traces contain Compose composition/recomposition slices
without shipping the large Perfetto binary in the production app.

Generate release Compose compiler reports plus connected jank evidence:

```powershell
.\scripts\collect_android_performance_evidence.ps1 -DeviceSerial emulator-5554
```

To generate compiler evidence without a connected benchmark run:

```powershell
.\scripts\collect_android_performance_evidence.ps1 -SkipBenchmarks
```

Generated runs are written beneath `androidApp/performance-evidence/<timestamp>`
and include a device/commit manifest, compiler stability and skippability
reports, Macrobenchmark JSON/results, and Perfetto traces. Generated run
directories are git-ignored; the checked-in
`androidApp/performance-evidence/README.md` defines the review and retention
contract.
