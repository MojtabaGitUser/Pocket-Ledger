...
class PocketLedgerDesktopDatabaseTest {
    @Test
    fun desktopDatabase_persistsRowsAcrossInstances() = runTest {
        val dbFile = File.createTempFile("pocket-ledger-desktop", ".db").apply { delete() }
        try {
            val firstDatabase = createPocketLedgerDatabase(dbFile.absolutePath)
            try {
            ...
            val secondDatabase = createPocketLedgerDatabase(dbFile.absolutePath)
            try {
